...
class Segment(BaseModel):
    ...
    key: str = Field(..., description="Redis key format: hnfm:seg:{item_id}:{run}:{seg}")
    item_id: int = Field(..., description="Hacker News item ID")
    run: int = Field(..., description="Run number for this item")
    ...
    processed_run_key: str = Field(..., description="Reference to the processed run key")
    ...
    audio_combined_path: Optional[str] = Field(default=None, description="Path to combined audio file")
    audio_ready: bool = Field(default=False, description="Whether audio is ready for playback")


class SegmentSection(BaseModel):
    """Model for individual audio sections within a segment"""

    key: str = Field(..., description="Redis key format: hnfm:seg:{item_id}:{run}:{seg}:sec:{section}")
    item_id: int = Field(..., description="Hacker News item ID")
    run: int = Field(..., description="Run number for this item")
    seg: int = Field(..., description="Segment number within the run")
    ...
    cleaned: bool = Field(default=False, description="Whether audio has been processed through studio-voice")
...
class SegmentsListResponse(BaseModel):
    ...
    item_id: int = Field(..., description="Hacker News item ID")
    run: int = Field(..., description="Run number")
    segments: List[SegmentSummary] = Field(..., description="List of segments with previews")
    pagination: Dict[str, int] = Field(..., description="Pagination info")
...
class BuildAudioResponse(BaseModel):
    ...
    section: Optional[int] = Field(default=None, description="Section number if mode is one")
...
class SectionsListResponse(BaseModel):
    ...
    run: int = Field(..., description="Run number")
    seg: int = Field(..., description="Segment number")
    sections: List[Dict[str, Any]] = Field(..., description="List of sections with metadata")
