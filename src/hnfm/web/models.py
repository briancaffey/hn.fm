...
class Segment(BaseModel):
    ...
    script_json: Optional[Dict[str, Any]] = Field(
        default=None,
        description=(
            "Structured script (content.llm_schemas.Script): writer-chosen "
            "section boundaries with per-section beat and visual_intent. None "
            "for pre-plans/08 segments, which fall back to the legacy splitter."
        ),
    )
